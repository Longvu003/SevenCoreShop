...
router.get("/thongke", async (req, res) => {
  try {
    ...
    const TongTienThang = await BestSellItemController.tongtientheothang(currentYear);
    ...
    return res.status(200).json({ message: "Nhập Dữ Liệu Thành Công", bestSellingProduct:bestSellingProduct,...TongTienNam, TongTienThang, TongDonHang: TongDonHang });
  } catch (error) {
