...
const ProductDetail = ({ navigation, route }) => {
  ...
  const addProductCart = async () => {
    try {
      ...
      if (!userId) {
        Alert.alert('Yêu cầu đăng nhập', 'Bạn cần đăng nhập để thêm vào giỏ hàng.', [
      ...
      const response = await axios.post(`${API__URL}/carts/addItemcart`, product, {
  ...
  const updateComment = async commentId => {
    ...
    if (!userEmail) {
      Alert.alert(
        'Yêu cầu đăng nhập',
        'Bạn cần đăng nhập để sửa bình luận.',
        [
          {
            text: 'Hủy',
            style: 'cancel',
  ...
  return (
    <ScrollView style={{ backgroundColor: 'white' }}>
      <View style={{ height: HEIGHT__SCREEN * 0.08, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Customheader leftIcon={require('../../../assets/imgs/back3.png')} />
        <TouchableOpacity style={{ marginRight: 24, marginTop: 24 }} onPress={() => addFavorite(item)}>
          <Image
